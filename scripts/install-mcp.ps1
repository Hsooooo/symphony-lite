# Symphony Lite MCP One-Liner Installer for Windows
# Usage: Invoke-WebRequest -Uri <url> -UseBasicParsing | Invoke-Expression

$ErrorActionPreference = "Stop"

$API_URL = if ($env:SYMPHONY_API_URL) { $env:SYMPHONY_API_URL } else { "http://localhost:18080/api/v1" }
$API_KEY = if ($env:SYMPHONY_API_KEY) { $env:SYMPHONY_API_KEY } else { "" }
$INSTALL_DIR = Join-Path $env:USERPROFILE ".symphony-mcp"
$VENV_DIR = Join-Path $INSTALL_DIR "venv"

Write-Host "🎵 Symphony Lite MCP Installer (Windows)" -ForegroundColor Cyan
Write-Host ""
...
if (-not $pythonCmd) {
    ...
    Write-Host "  1. https://python.org/downloads 에서 Python 3.12 다운로드"
    Write-Host "  2. 설치 시 'Add Python to PATH' 체크!"
    Write-Host "  3. PowerShell을 재시작 후 다시 실행"
    exit 1
}

$PYTHON = $pythonCmd.Source
Write-Host "✅ Python 확인됨: $PYTHON" -ForegroundColor Green
...
# Install packages
Write-Host "📦 패키지 설치 중 (mcp, httpx)..." -ForegroundColor Cyan
& $VENV_PIP install -q --upgrade pip
& $VENV_PIP install -q mcp==1.6.0 httpx==0.27.2

# Write MCP server script
$SERVER_PY = Join-Path $INSTALL_DIR "server.py"
@'
import asyncio
import argparse
import os
...
if __name__ == "__main__":
    asyncio.run(main())
'@ | Set-Content -Path $SERVER_PY -Encoding UTF8

Write-Host "✅ MCP 서버 스크립트 설치 완료: $SERVER_PY" -ForegroundColor Green

# Write env file
$ENV_FILE = Join-Path $INSTALL_DIR ".env"
"SYMPHONY_API_URL=$API_URL`nSYMPHONY_API_KEY=$API_KEY" | Set-Content -Path $ENV_FILE -Encoding UTF8

Write-Host "✅ 환경 변수 파일 생성: $ENV_FILE" -ForegroundColor Green

# Generate configs
$KIMI_CONFIG = Join-Path $INSTALL_DIR "mcp-kimi.json"
@"
{
  "mcpServers": {
    "symphony-lite": {
      "command": "$($VENV_PYTHON -replace '\\', '\\')",
      "args": ["$($SERVER_PY -replace '\\', '\\')"],
      "env": {
...
"@ | Set-Content -Path $KIMI_CONFIG -Encoding UTF8

$CLAUDE_CONFIG = Join-Path $INSTALL_DIR "mcp-claude.json"
@"
{
  "mcpServers": {
    "symphony-lite": {
...
"@ | Set-Content -Path $CLAUDE_CONFIG -Encoding UTF8

Write-Host ""
Write-Host "🎉 설치 완료!" -ForegroundColor Green
Write-Host ""
Write-Host "📁 설치 위치: $INSTALL_DIR" -ForegroundColor Yellow
Write-Host ""
Write-Host "🔗 연동 방법:" -ForegroundColor Cyan
Write-Host ""
Write-Host "  [Kimi Code CLI]" -ForegroundColor Magenta
Write-Host "  kimi mcp add --transport stdio symphony-lite -- ``"
Write-Host "    $VENV_PYTHON $SERVER_PY"
Write-Host ""
Write-Host "  [Claude Code]" -ForegroundColor Magenta
Write-Host "  ~/.mcp.json 에 아래 내용을 추가하세요:"
Write-Host "  $CLAUDE_CONFIG"
Write-Host ""
Write-Host "  [또는 직접 실행]" -ForegroundColor Magenta
Write-Host "  $VENV_PYTHON $SERVER_PY"
Write-Host ""
